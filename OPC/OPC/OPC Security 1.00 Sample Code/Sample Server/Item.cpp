...
XXXItem::XXXItem( XXXGroup *parent)
{
    ...
    _tcscpy(m_szValueCache, szBase);
    _tcscat(m_szValueCache, _T("\\acl.test"));
    // %END_OPC_SEC_MODIFIED%
}
...
HRESULT XXXItem::GetValue( OPCDATASOURCE ds, VARIANT * v, WORD *q, FILETIME*t)
{
    ...
    // %OPC_SEC_MODIFIED%
    // before proceeding any further, verify that caller has access permissions
	if (m_Parent && m_Parent->GetParent())
    {
#ifndef USE_CACHED_THREAD_TOKEN  // use private ACL for OPC item:
        r1 = m_Parent->GetParent()->CheckAccessRights(XXXServer::ReadItem);
        if (FAILED(r1)) {
            return r1;  // oops, no access and bye....
        }
#else // USE_CACHED_THREAD_TOKEN  // use NT system ACL with cached thread token for OPC item:

        // as a test, this code simply tries to read-open a file "acl.test"
        // in the server's home directory, using the stored thread token context.
        // Change its ACL with Explorer property page to control if user has access
        ::SetThreadToken(NULL, m_Parent->GetParent()->GetCachedThreadHandle());
        FILE *hTest = _tfopen(m_szValueCache, _T("r"));
        r1 = HRESULT_FROM_WIN32(GetLastError());
        if (hTest != NULL) {
            fclose(hTest);
        }
