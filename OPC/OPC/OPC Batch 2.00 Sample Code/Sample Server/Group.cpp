...
//        10/20/99 acc   Add IsAdvised
//        07/27/00 acc/md fix revised update rate calculation.
//
//
...
STDMETHODIMP XXXGroup::QueryInterface( REFIID iid, LPVOID* ppInterface)
{
	...
	else if ( iid == IID_IConnectionPointContainer)		//V2.0
	{
		...
		if(m_pCPC == 0) 
		{
			...
			//
			pCP = new IXXXCP(IID_IOPCDataCallback);
			// later check for error/null...

			// Create the CPC 
			// (this is the actual tear-off we will Addref and return)
			//
			m_pCPC = new IXXXCPC(this, IID_IOPCDataCallback);
			// later check for error/null...
...
// XXXGroup::IsAdvised
//
// This method determines if the group has an existing connection point advise.
//
BOOL		XXXGroup::IsAdvised( void )
{
	...
	// check is connection point exists
	//
	if(!m_pCPC) return result;
	m_pCPC->FindConnectionPoint(IID_IOPCDataCallback, (IConnectionPoint**)&pCP);
	if(!pCP) return result;

	// and if so, is it advised
	//
	if(pCP->m_Callback) result = TRUE;
...
///////////////////////////////////////
// XXXGroup::SetUpdateRate
//
HRESULT XXXGroup::SetUpdateRate(DWORD dwRequestedUpdateRate)
{
	DWORD temp = dwRequestedUpdateRate;
	DWORD MinRate = 250;	// pretend 250ms is our minimum rate and also our resolution

	// If non-zero requested rate 
	if(temp)
	{
		//then compute best rate we can do (i.e. round 'down' from multiple of 250ms)...

		temp += MinRate - 1;		// if temp=1000 -> ((1000+249)/250)+250 = 1000
	...
	{
		// else if 0 requested then just go as fast as we can.
		temp = MinRate;
	}
	m_dwRevisedRate = temp;

	if(m_dwRevisedRate != dwRequestedUpdateRate) return OPC_S_UNSUPPORTEDRATE;
	return S_OK;
}
