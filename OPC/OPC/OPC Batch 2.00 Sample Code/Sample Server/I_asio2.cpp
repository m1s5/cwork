...
//       10/20/99 acc   add error check to get/set enable
//       02/22/00 acc   fix for proper error return from read and write if bad handle.
//
    ...
    )
{
	...
	if(!g.IsAdvised()) return CONNECT_E_NOCONNECTION;
